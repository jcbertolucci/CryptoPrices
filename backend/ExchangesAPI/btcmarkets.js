...
class BtcMarkets{
  ...
  */
  ...
  async getBalance(){
    try{
      const accountBalances = await this.client.getAccountBalances();
  ...
  async getOrderHistory(){
    try{
      const orderHistory = await this.client.getOrderHistory(this.instrument, this.currency, this.limit, null);
  ...
  async createOrder(){
    try{
      ...
      const createdOrder = await this.client.createOrder(this.instrument, this.currency, this.price * numberConverter, this.volume * numberConverter, this.orderSide, this.orderType, this.clientId);
      return createdOrder;
    }catch(error){
      return error;
    }
